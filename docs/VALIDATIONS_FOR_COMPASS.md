# Валидации MongoDB для Compass

## Инструкция по применению валидаций в MongoDB Compass

1. Откройте MongoDB Compass
2. Выберите базу данных `auen` (или вашу базу данных)
3. Для каждой коллекции выполните следующие шаги:

### Для коллекции `users`:
1. Кликните на коллекцию `users`
2. Перейдите на вкладку "Validation"
3. Вставьте следующую валидацию:
...
```json
{
  "$jsonSchema": {
    ...
    "properties": {
      ...
      "isBlocked": {
        "bsonType": "bool",
        "description": "Статус блокировки пользователя (по умолчанию false)"
      },
      "bio": {
        "bsonType": ["string", "null"],
        "maxLength": 500,
        "description": "Описание профиля, максимум 500 символов (опционально)"
      },
      "website": {
        "bsonType": ["string", "null"],
        "pattern": "^https?://.+",
        "description": "URL сайта пользователя (опционально)"
      },
      "instagram": {
        "bsonType": ["string", "null"],
        "pattern": "^[a-zA-Z0-9._]+$",
        "description": "Username Instagram (опционально)"
      },
      "telegram": {
        "bsonType": ["string", "null"],
        "pattern": "^[a-zA-Z0-9_]+$",
        "description": "Username Telegram (опционально)"
      },
      "vk": {
        "bsonType": ["string", "null"],
        "pattern": "^[a-zA-Z0-9._]+$",
        "description": "Username VK (опционально)"
      },
      "youtube": {
        "bsonType": ["string", "null"],
        "pattern": "^[a-zA-Z0-9._-]+$",
        "description": "Username YouTube (опционально)"
      },
      "createdAt": {
        "bsonType": ["date", "null"],
...
```json
{
  "$jsonSchema": {
    ...
    "properties": {
      ...
      "featured": {
        "bsonType": "bool",
        "description": "Показывать ли объявление в рекламной галерее на главной странице (по умолчанию false)"
      },
      "bookings": {
        "bsonType": "array",
        "items": {
          ...
          "properties": {
            ...
            "startTime": {
              "bsonType": ["date", "null"],
              "description": "Время начала аренды (опционально)"
            },
            "endTime": {
              "bsonType": ["date", "null"],
              "description": "Время окончания аренды (опционально)"
            },
            "period": {
              "bsonType": "string",
              "enum": ["hour", "day", "week", "month"],
              "description": "Тип периода аренды"
            },
            "price": {
              "bsonType": "double",
              "minimum": 0,
        ...
        "description": "Массив бронирований для этого объявления"
      },
      "createdAt": {
        "bsonType": ["date", "null"],
...
1. Кликните на коллекцию `messages`
2. Перейдите на вкладку "Validation"
3. Вставьте следующую валидацию:
...
### Для коллекции `reviews`:
1. Кликните на коллекцию `reviews`
2. Перейдите на вкладку "Validation"
3. Вставьте следующую валидацию:
...
### Для коллекции `userreviews` (если используется):
1. Кликните на коллекцию `userreviews`
2. Перейдите на вкладку "Validation"
3. Вставьте следующую валидацию:
...
```json
{
  "$jsonSchema": {
    ...
    "properties": {
      ...
      "reviewedUserId": {
        "bsonType": "objectId",
        "description": "ID пользователя, на которого оставляют отзыв"
      },
      "rating": {
        "bsonType": "int",
        "minimum": 1,
...
1. **Validation Level**: Установите `moderate` (проверяет только новые и обновленные документы) или `strict` (проверяет все документы)
2. **Validation Action**: Установите `error` (блокирует невалидные документы) или `warn` (только предупреждает)
3. Все валидации используют `additionalProperties: true`, что позволяет добавлять дополнительные поля
4. Поля с `["type", "null"]` являются опциональными
5. После применения валидации, существующие документы могут не соответствовать схеме - проверьте их вручную
